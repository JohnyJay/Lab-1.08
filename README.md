...
## EXAMPLES OF USAGE OF INTLIST INTERFACE (GENERATED BY ME)

The `IntArrayList` class and the `IntVector` class behave similarly and both implement the `IntList` Interface. They are used to add integer numbers to an array that will grow in size when needed.

The `IntArrayList` class grows somewhat slower. It is particularly efficient when the frequency of adding new elements is low or when an increase in elements to be added is not expected.

On the other hand, the `IntVector` class doubles it size each time it is required to do so, which will make it particularly more efficient when a large number of elements need to be added and we expect the rate of addition to increase over time.
